...
def detect_arcus_opencv(image: Image.Image) -> Dict:
    ...
    if len(segment_intensities) >= 8:
        ...
        is_uniform_ring = (segment_cv < 0.32 and segments_bright >= 9)
    else:
        segment_cv = 1.0
        segment_range = 0
        segments_bright = 0
    ...
    if is_dark_image:
        BRIGHTNESS_DIFF_MIN = 25
        CONTRAST_MIN = 1.35
        RING_BRIGHTNESS_MIN = 90
    else:
        BRIGHTNESS_DIFF_MIN = 35
        CONTRAST_MIN = 1.40
        RING_BRIGHTNESS_MIN = 150
    ...
    strong_arcus = (
        ...
        segments_bright >= 10 and segment_cv < 0.12
    )
    
    moderate_arcus = (
        intensity_diff > (BRIGHTNESS_DIFF_MIN + 15) and
        contrast_ratio > (CONTRAST_MIN + 0.15) and
        has_uniform_pattern and
        has_smooth_ring and
        not ring_similar_to_sclera and
    ...
    mild_arcus = (
        intensity_diff > BRIGHTNESS_DIFF_MIN and
        contrast_ratio > CONTRAST_MIN and
        segment_cv < 0.22 and  # Tightened from 0.28
        has_smooth_ring and
        not ring_similar_to_sclera and
        segments_bright >= 8
    )
    ...
    print(f"  Segment CV:         {segment_cv:6.3f} (need: <0.15)")
    print(f"  Segment Range:      {segment_min:.0f} - {segment_max:.0f} (Δ{segment_range:.0f})")
    print(f"  Bright Segments:    {segments_bright}/12 (need: ≥10)")
